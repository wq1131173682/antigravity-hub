    ...
    std::thread::spawn(move || {
        let result = (|| -> Option<String> {
            ...
            if let Ok(resp) = client.get(VERSION_URL).send() {
                if let Ok(text) = resp.text().map(|s| s.to_string()) {
...
/// Uses known stable version as base, optionally updates from remote.
fn resolve_version_config() -> (VersionConfig, VersionSource) {
    ...
    // Try Remote Version (best-effort; failure is silently ignored)
