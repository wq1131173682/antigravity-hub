...
pub fn get_tray_texts(lang: &str) -> TrayTexts {
    ...
    TrayTexts {
        switch_next: t.get("switch_next").cloned().unwrap_or_else(|| "Switch to Next Key".to_string()),
        refresh_current: t.get("refresh_current").cloned().unwrap_or_else(|| "Refresh Quota".to_string()),
