...
export default function CodexIntegration() {
  ...
  useEffect(() => {
    if (selectedPlatformId) {
      fetchModels(selectedPlatformId).then(() => {
        // Use getState() to read fresh state (closure variable may be stale)
  ...
  const handleApply = useCallback(async () => {
    ...
    // Find the selected platform's path_prefix
...
openai_base_url = "http://${config?.proxy_host || '127.0.0.1'}:${config?.proxy_port || 8080}/${(platforms.find(p => p.id === selectedPlatformId)?.path_prefix) || 'openai'}/v1"`}
            </pre>
          </div>
