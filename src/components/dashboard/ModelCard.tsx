...
export function ModelCard({ model, usageEntries, defaultExpanded = false }: ModelCardProps) {
  ...
  // MODEL_CONFIG is keyed by model name (`gemini-3-flash`, `claude-opus-4-6-thinking`).
  // `model.id` is a v4 UUID generated by the Rust backend, so looking the icon up
  // by id never matched and every card fell back to the generic "M" placeholder.
  const modelConfig = MODEL_CONFIG[model.model_name.toLowerCase()];
      ...
      {expanded && usageLoaded && (
        <div className="border-t border-gray-100/60 dark:border-base-300/60">
          {sortedEntries.map(u => {
            ...
            // Tint steps must be from Tailwind's default opacity scale. The
            // previous `dark:bg-red-900/8` and `dark:bg-amber-900/8` compiled to
            // nothing (8 is not a stock step), so in dark mode an over-quota row
            // had no background highlight at all - only the status dot differed.
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="h-1 w-full rounded-full overflow-hidden bg-gray-100 dark:bg-base-300">
                      <div
                        className={`h-full w-full origin-left rounded-full transition-transform duration-300 ${STATUS_BAR_SOLID[status5h]}`}
                        style={{ transform: `scaleX(${model.per_5hour > 0 ? Math.min(1, u.five_hour.count / model.per_5hour) : 0})` }}
                      />
                    </div>
                  </div>
                  ...
                  <div className="w-20 flex-shrink-0">
                    ...
                    <div className="h-1 w-full rounded-full overflow-hidden bg-gray-100 dark:bg-base-300">
                      <div
                        ...
                        style={{
                          transform: `scaleX(${model.per_day > 0 ? Math.min(1, u.day.count / model.per_day) : 0})`,
                  ...
                  <div className="w-20 flex-shrink-0">
                    ...
                    <div className="h-1 w-full rounded-full overflow-hidden bg-gray-100 dark:bg-base-300">
                      <div
                        className="h-full w-full origin-left rounded-full transition-transform duration-300"
                        style={{
                          transform: `scaleX(${model.per_month > 0 ? Math.min(1, u.month.count / model.per_month) : 0})`,
